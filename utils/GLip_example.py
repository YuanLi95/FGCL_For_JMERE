...
def process(data_set_path):
    ...
    with jsonlines.open("../no_none_unified_tags_txt/val_entity.json", "a") as f:
...
def find_no_text(train_path):
    x_1 = ['San', 'Diego', 'Padres', ':', 'Reyes', ',', 'Paddack', ',', 'Radke', 'Nominated', 'For', 'MiLB', 'Awards',
           '#', 'Padres']
    x_2 = ['RT', '@ManuelDuarte24', ':', 'The', 'scene', 'between', 'Luke', 'and', 'Yoda', 'in', 'The', 'Last', 'Jedi',
           'is', 'a', 'top', '5', 'scene', 'in', 'the', 'Star', 'Wars', 'saga']
    x_3 = ['RT', '@XSovietNews', ':', 'Russia', "'s", 'Foreign', 'Ministry', 'spokeswoman', 'Maria', 'Zakharova',
           'captioned', 'this', '"', 'With', 'my', 'colleague', 'from', 'work', '.', '"']
    x_1 = " ".join(x_1)
    x_2 = " ".join(x_2)
    x_3 = " ".join(x_3)
    Pro = "Q: Please identify Person, Location, Organization and Miscellaneous entity from the text \n"
    ...
    # Prp_all= Prp_1
    object_token_list = []
    with open("../no_none_unified_tags_txt/train_entity.json", "r+") as f:
        for item in jsonlines.Reader(f):
            object_token_list.append(item["token"])

    openai.api_key = "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"
    file = json.load(open(train_path))

    with jsonlines.open("../no_none_unified_tags_txt/train_entity.json", "a") as f:
        for index, text in enumerate(file):
            test_text = text["token"]
            print(index)
            if test_text in object_token_list:
                print(test_text)
                continue
